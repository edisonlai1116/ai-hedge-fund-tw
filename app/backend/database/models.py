...
class PodcastEpisode(Base):
    """Table to store transcribed podcast episodes and transcripts"""
    __tablename__ = "podcast_episodes"
    
    id = Column(String(100), primary_key=True, index=True)  # Episode ID or URL
    title = Column(String(250), nullable=False)
    published_date = Column(String(100), nullable=True)
    audio_url = Column(Text, nullable=True)
    transcript = Column(Text, nullable=True)  # Whisper Traditional Chinese transcript
    created_at = Column(DateTime(timezone=True), server_default=func.now())


class PodcastTicker(Base):
    """Table to store individual stock tickers mentioned in podcast episodes"""
    __tablename__ = "podcast_tickers"
    
    id = Column(Integer, primary_key=True, index=True)
    episode_id = Column(String(100), ForeignKey("podcast_episodes.id"), nullable=False, index=True)
    ticker = Column(String(50), nullable=False, index=True)
    context = Column(Text, nullable=True)  # Extract paragraph of context where ticker was mentioned
    sentiment = Column(String(50), nullable=True)  # e.g., "Bullish", "Bearish", "Neutral"
    sentiment_score = Column(Integer, nullable=True)  # 0 to 100
    confidence = Column(Integer, nullable=True)  # 0 to 100


class CustomSentiment(Base):
    """Table to store user-submitted analyst sentiment URLs (Threads, YouTube, Web)"""
    __tablename__ = "custom_sentiments"
    
    id = Column(Integer, primary_key=True, index=True)
    url = Column(Text, nullable=False)
    analyst_name = Column(String(100), nullable=True)
    ticker = Column(String(50), nullable=False, index=True)
    sentiment_label = Column(String(50), nullable=False)  # Bullish, Bearish, Neutral
    sentiment_score = Column(Integer, nullable=False)  # 0 to 100
