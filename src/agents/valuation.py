...
def valuation_analyst_agent(state: AgentState, agent_id: str = "valuation_analyst_agent"):
    ...
    for ticker in tickers:
        ...
        # Benjamin Graham total defensive value
        book_value = (most_recent_metrics.market_cap / most_recent_metrics.price_to_book_ratio) if most_recent_metrics.price_to_book_ratio else 0.0
        graham_val = calculate_graham_value(li_curr.net_income, book_value)

        # Piotroski F-Score financial health analysis
        f_score = calculate_f_score(li_curr, li_prev, most_recent_metrics)

        method_values = {
            "dcf": {"value": dcf_val, "weight": 0.30},
            "owner_earnings": {"value": owner_val, "weight": 0.30},
            "ev_ebitda": {"value": ev_ebitda_val, "weight": 0.15},
            "residual_income": {"value": rim_val, "weight": 0.10},
            "graham": {"value": graham_val, "weight": 0.15},
        }
        ...
        # F-Score adjustment to reward strong fundamentals and penalize weak ones
        if f_score >= 7:
            weighted_gap += 0.05
        elif f_score <= 3:
            weighted_gap -= 0.10
        ...
        # Enhanced reasoning with DCF scenario, Graham, and F-Score details
        reasoning = {}
        ...
        # Add overall DCF scenario and F-Score summary
        ...
        reasoning["financial_health_score"] = {
            "piotroski_f_score_proxy": f"{f_score}/9",
            "interpretation": "Excellent (Fundamental strength)" if f_score >= 7 else "Weak (Value trap risk)" if f_score <= 3 else "Moderate"
        }
...
def calculate_graham_value(net_income: float | None, total_book_value: float | None) -> float:
    if not net_income or not total_book_value or net_income <= 0 or total_book_value <= 0:
        return 0.0
    return (22.5 * net_income * total_book_value) ** 0.5


def calculate_f_score(li_curr, li_prev, most_recent_metrics) -> int:
    score = 0
    # 1. Net Income > 0
    if getattr(li_curr, 'net_income', None) and li_curr.net_income > 0:
        score += 1
    # 2. Free Cash Flow > 0 (as proxy for CFO)
    if getattr(li_curr, 'free_cash_flow', None) and li_curr.free_cash_flow > 0:
        score += 1
    # 3. CFO > Net Income (FCF > Net Income)
    if getattr(li_curr, 'free_cash_flow', None) and getattr(li_curr, 'net_income', None) and li_curr.free_cash_flow > li_curr.net_income:
        score += 1
    # 4. ROE > 0 or positive ROE
    roe = getattr(most_recent_metrics, 'return_on_equity', None)
    if roe and roe > 0:
        score += 1
    # 5. Debt to Equity is low
    dte = getattr(most_recent_metrics, 'debt_to_equity', None)
    if dte is not None and dte < 1.5:
        score += 1
    # 6. Current Ratio > 1.0
    cr = getattr(most_recent_metrics, 'current_ratio', None)
    if cr and cr > 1.0:
        score += 1
    # 7. Positive Revenue Growth
    rg = getattr(most_recent_metrics, 'revenue_growth', None)
    if rg and rg > 0:
        score += 1
    # 8. Positive Earnings Growth
    eg = getattr(most_recent_metrics, 'earnings_growth', None)
    if eg and eg > 0:
        score += 1
    # 9. Gross Profit Margin is positive/healthy
    opm = getattr(most_recent_metrics, 'operating_margin', None)
    if opm and opm > 0:
        score += 1
    return score
